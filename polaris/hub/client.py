...
class PolarisHubClient(OAuth2Client):
    ...
    def upload_benchmark(self, benchmark: BenchmarkSpecification):
        ...
        if benchmark.name is None or benchmark.owner is None:
            raise PolarisHubError(
                "Benchmark name and owner must be set to upload benchmark to the Polaris Hub."
            )
        temp_dir = tempfile.TemporaryDirectory().name
        json_path = benchmark.to_json(temp_dir)
        with open(json_path) as f:
            benchmark_json = json.load(f)

        # lu: temporary walk around for posting with none
        benchmark_json = NullNone2String(benchmark_json)
...
def NullNone2String(ob):
    """Convert all None to string"""
    l = {}
    for k, v in ob.items():
