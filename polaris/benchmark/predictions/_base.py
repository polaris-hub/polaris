...
class BenchmarkPredictions(BaseModel):
    ...
    def validate_predictions(cls, data):
        ...
        data["predictions"] = cls._normalize_predictions(vals, target_cols)
        return data

    @classmethod
    def _normalize_predictions(cls, vals: IncomingPredictionsType, target_cols: list[str]) -> PredictionsType:
        ...
        if cls._is_multi_task_single_test_set(vals, target_cols):
            return {"test": vals}
        #     # vals = cls._convert_lists_to_numpy(vals)
    ...
    def _is_multi_task_single_test_set(cls, vals, target_cols) -> bool:
        ...
        return isinstance(vals, dict) and all(k in target_cols for k in vals)
